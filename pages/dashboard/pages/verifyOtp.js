...
console.log(';;;;;;;;;;;;;' , data);
    ...
    setLoading(false);
    data.success === true
      ? saveUserInCookies(data?.data)
      : setError(" رمز التحقق غير صحيح أو انتهت المدة الزمنية");

  };

  const saveUserInCookies = async (data) => {
    const { permissions, ...userDataWithoutPermissions } = data;

    // Save user data (without permissions) in cookies
    Cookies.set("oboor_admin", JSON.stringify(userDataWithoutPermissions), { expires: 7 });
  
    // Save permissions in local storage
    localStorage.setItem("permissions", JSON.stringify(permissions));
    console.log(data);
    if (data.user_type == "SALE") {
