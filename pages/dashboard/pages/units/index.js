    ...
    >
      <Card style={{ width: "50%", minWidth: "350px", maxWidth: "600px" }}>
        <Card.Body className='text-center'>
          <h4>هل انت متاكد من حذف الوحدة : {userData?.name}</h4>
...
const Orders = () => {
  ...
  const deleteRow = async (id) => {
    setLoading(true);
    const res = await fetch(`http://192.34.59.161/api/v1/units/${id}/`, {
      method: "DELETE",
      headers: {
        // "Content-Type": "application/json",
        Authorization: `${token}`,
  ...
  const addPackage = async (e, data) => {
    ...
    const res = await fetch(`http://192.34.59.161/api/v1/units/${!isAddNew ? userData.id + '/' : ""}`, {
  ...
  return (
    <>
      ...
      <PageHeader title='الوحدات' item='شركة وحيد' active_item='الوحدات' />
...
const AddPackagePopup = ({ setAddPackagePopup, addPackage, loading  ,userData, isAddNew }) => {
  ...
  const [title, setTitle] = useState(!isAddNew ? userData.name : null);
    ...
    >
      <Card style={{ width: "50%", minWidth: "350px", maxWidth: "600px" }}>
        <Card.Header>
          ...
          {isAddNew ? <h3>وحدة جديدة</h3> : <h3>تعديل الوحدة </h3>}
        </Card.Header>
        <Card.Body>
          <Form
            onSubmit={(e) =>
          ...
          >
            ...
            <div className='d-flex gap-4'>
              ...
              >
                {loading ? "جار الإضافة..." : isAddNew ? "إضافة وحدة جديدة" : "تعديل"}
              </Button>
              <Button
