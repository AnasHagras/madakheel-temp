    ...
    >
      <Card style={{ width: "50%", minWidth: "350px", maxWidth: "600px" }}>
        <Card.Body className='text-center'>
          <h4>هل انت متاكد من حذف التصنيف : {userData?.name_ar}</h4>
...
const Orders = () => {
  const [data, setData] = useState([
    {
      ...
      "name_ar": "Fruits",
      "name_en": "Fruits",
      "description": "This is category descritpion",
      "image": null,
      "created_at": "2024-05-20T21:18:22.280391+03:00",
      "updated_at": "2024-05-20T21:18:22.280391+03:00"
    },
    {
      "id": 2,
      "name_ar": "مأكولات",
      "name_en": "Food",
      "description": "This is category for food.",
      "image": "http://localhost:8000/media/product_categories/orange.png",
      "created_at": "2024-05-20T02:07:04.073794+03:00",
      "updated_at": "2024-05-20T21:19:15.835756+03:00"
    }
  ]);
  ...
  const deleteRow = async (id) => {
    setLoading(true);
    const res = await fetch(`http://192.34.59.161/api/v1/categories/${id}/`, {
      method: "DELETE",
      headers: {
        // "Content-Type": "application/json",
        Authorization: token,
  ...
  const addItem = async (e, data) => {
    ...
    const res = await fetch("http://192.34.59.161/api/v1/categories/", {
      method: "POST",
      body: fd,
      headers: {
        // "Content-Type": "application/json",
  ...
  const editItem = async (e, data) => {
    ...
    fd.append("name_en", title);
    fd.append("description", desc);
    files && fd.append("image", files[0])


    const res = await fetch(`http://192.34.59.161/api/v1/categories/${userData.id}/`, {
      method: "PUT",
      body: fd,
      headers: {
        // "Content-Type": "application/json",
  ...
  return (
    <>
      ...
      <PageHeader title='الأقسام' item='شركة وحيد' active_item='الأقسام' />
...
const AddItemPopup = ({ setAddItemPopup, addItem, loading }) => {
        ...
        <Card.Body>
          ...
          >
            ...
            <div className='d-flex gap-4'>
              ...
              >
                {loading ? "جار الإضافة..." : "إضافة قسم جديد"}
              </Button>
              <Button
...
const EditItemPopup = ({ setEditItemPopup, editItem, loading , userData}) => {
  const [price, setPrice] = useState(null);
  const [profit, setProfit] = useState(null);
  const [commission, setCommission] = useState(null);
  const [quantity, setQuantity] = useState(null);
  const [files, setFiles] = useState(null);
  const [title, setTitle] = useState(userData?.name_ar);
      ...
      <Card style={{ width: "50%", minWidth: "350px", maxWidth: "600px" }}>
        ...
        <Card.Body>
          ...
          >
            ...
            <Form.Group className='text-start form-group'>
              <Form.Label> تعديل صورة  </Form.Label>
              <Form.Control
                className='form-control'
                name='files'
                type='file'
            ...
            <div className='d-flex gap-4'>
              <Button
                ...
                className='btn ripple btn-main-primary btn-block mt-2 disabled:opacity-50 '
              >
                {loading ? "جار التعديل..." : "  تعديل القسم"}
              </Button>
              <Button
                onClick={() => setEditItemPopup(false)}
                type='button'
                className='btn ripple btn-main-primary btn-block mt-2 '
              >
                الغاء
